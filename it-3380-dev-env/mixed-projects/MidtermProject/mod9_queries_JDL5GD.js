...
//2. Calculate the total payments each customer has made. Display customer name 
//and the total payment. (98 results)
...
//3 Calculate the number of payments each customer has made. Display customer name 
//and the number of payments. (98 results in total)
db.customers.aggregate([
    { $unwind: "$payments" },
...
//5. What are the 10 most expensive orders? Display the Customer name and order 
//total and display results in descending order by order total.

db.orders.aggregate([
    { $unwind: "$orderDetails" },
    { $group: { _id: "$_id", customerName: { $addToSet: "$customerName" }, orderTotal: { $sum: { $multiply: ["$orderDetails.priceEach", "$orderDetails.quantityOrdered"] } } } },
    { $sort: { orderTotal: -1 } },
    { $limit: 10 },
])


//6. What is the sales value of each product in stock? Sale value is calculated by 
//multiplying quantity in stock by MSRP. Display the product name and the sales
//value in descending order based on sales value. (110 results)
...
//8. How many customers does each sales rep have? Display the Sales rep name 
